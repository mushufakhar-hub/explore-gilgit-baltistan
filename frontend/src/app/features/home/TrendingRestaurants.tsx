...
export default function TrendingRestaurants() {
  const { data, error, isError, isLoading, refetch } = useListings({ categoryGroupId: 'eat-drink', limit: 3, sort: 'created_at_desc' })

  return (
    <section className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.35em] text-slate-500">Trending restaurants</p>
          <h2 className="mt-2 text-3xl font-semibold text-slate-900">Dine where locals and travelers love</h2>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {data.items.map((listing) => (
            <ListingCard
              ...
              price={listing.price}
              categoryGroup={(listing.category_group ?? 'eat-drink') as any}
